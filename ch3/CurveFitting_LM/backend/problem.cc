...
bool Problem::Solve(int iterations) {
    ...
    while (!stop && (iter < iterations)) {
        ...
        //**********************modified beginning***********************
        ofstream lambda_data("../../data/alg2_lambda_data.txt", ios_base::app);
        lambda_data << iter << "\t" << currentLambda_ << endl;
        lambda_data.close();
        //***********************modified ending************************
        ...
        {
            // setLambda (H + lamdaD^TD)*deltax = b
...
void Problem::ComputeLambdaInitLM() {
    ni_ = 2.;   // ppt page 20 -------   v = 2  lambda *= ni_
    currentLambda_ = -1.;
    currentChi_ = 0.0;
...
bool Problem::IsGoodStepInLM() {
    ...
    assert(Hessian_.rows()== Hessian_.cols() && "Hessian is not square");
    for (ulong i = 0; i < size; i++) {
        max_diagonal = std::max(fabs(Hessian_(i, i)), max_diagonal);
    }
    
    scale = delta_x_.transpose() * (currentLambda_ * max_diagonal * delta_x_ + b_); // 1end


    // algorithm 3 in 4.1.1
    // scale = delta_x_.transpose() * (currentLambda_ * delta_x_ + b_); // 3end
