...
using Line = std::pair<Eigen::Vector4d, Eigen::Vector4d>;  // 应该是两个点连成一条线的意思
...
void CreatePointsLines(Points& points, Lines& lines)
{
    ...
    while(!f.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            if(!isHistoryPoint)
                points.push_back(pt0);  // 如果不是重复点，就存入 points 中
    ...
    // save points
    save_points("all_points.txt", points);  // 将点存储
}
