...
void Problem::SetOrdering() {
    ...
    for (auto vertex: verticies_) {
        ...
        if (IsPoseVertex(vertex.second)) {
            std::cout << vertex.second->Id() << " pose order: " << vertex.second->OrderingId() << std::endl;
        }
    }
...
void Problem::TestMarginalize() {
    ...
    // 将 row i 移动矩阵最下面
    /// @details YWY 先移动的目的是，不论被边缘化的vertex在哪里
    // 后面的边缘化都只对最后几行进行操作就可以
    Eigen::MatrixXd temp_rows = H_marg.block(idx, 0, dim, reserve_size);
    Eigen::MatrixXd temp_botRows = H_marg.block(idx + dim, 0, reserve_size - idx - dim, reserve_size);
    H_marg.block(idx, 0, reserve_size - dim - idx, reserve_size) = temp_botRows;
